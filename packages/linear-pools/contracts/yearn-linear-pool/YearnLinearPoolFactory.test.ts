...
describe('YearnLinearPoolFactory', function () {
  ...
  beforeEach('deploy factory & tokens', async () => {
    ...
    factory = await deployPackageContract('YearnLinearPoolFactory', {
      args: [
        vault.address,
        ZERO_ADDRESS,
        queries.address,
  ...
  async function createPool(): Promise<Contract> {
    ...
    const yvDAI = await tokens.getTokenBySymbol('yvDAI');
    ...
    expectEvent.inReceipt(receipt, 'YearnLinearPoolCreated', {
      pool: event.args.pool,
      protocolId: YEARN_PROTOCOL_ID,
    });

    return getPackageContractDeployedAt('YearnLinearPool', event.args.pool);
  }
  ...
  describe('constructor arguments', () => {
    ...
    it('sets a rebalancer as the asset manager', async () => {
      ...
      const rebalancer = await getPackageContractDeployedAt('YearnLinearPoolRebalancer', assetManager);
    ...
    it('sets wrapped token', async () => {
      const yvDAI = await tokens.getTokenBySymbol('yvDAI');
      expect(await pool.getWrappedToken()).to.equal(yvDAI.address);
    });
  ...
  describe('protocol id', () => {
    it('should not allow adding protocols without permission', async () => {
      await expect(factory.registerProtocolId(YEARN_PROTOCOL_ID, 'Yearn')).to.be.revertedWith('BAL#401');
    });

    context('with no registered protocols', () => {
      it('should revert when asking for an unregistered protocol name', async () => {
        await expect(factory.getProtocolName(YEARN_PROTOCOL_ID)).to.be.revertedWith('Protocol ID not registered');
      });
    });
