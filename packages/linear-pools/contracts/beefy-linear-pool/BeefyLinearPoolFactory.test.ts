...
describe('BeefyLinearPoolFactory', function () {
  ...
  beforeEach('deploy factory & tokens', async () => {
    ...
    factory = await deployPackageContract('BeefyLinearPoolFactory', {
      args: [
        vault.address,
        ZERO_ADDRESS,
        queries.address,
  ...
  async function createPool(): Promise<Contract> {
    ...
    expectEvent.inReceipt(receipt, 'BeefyLinearPoolCreated', {
      pool: event.args.pool,
      protocolId: BEEFY_PROTOCOL_ID,
    });

    return getPackageContractDeployedAt('BeefyLinearPool', event.args.pool);
  }
  ...
  describe('constructor arguments', () => {
    ...
    it('registers tokens in the vault', async () => {
      ...
      expect(poolTokens.tokens).to.include(yvDAI.address);
    ...
    it('sets a rebalancer as the asset manager', async () => {
      ...
      const rebalancer = await getPackageContractDeployedAt('BeefyLinearPoolRebalancer', assetManager);
    ...
    it('sets wrapped token', async () => {
      const mooDAI = await tokens.getTokenBySymbol('mooDAI');
      expect(await pool.getWrappedToken()).to.equal(mooDAI.address);
    });
  ...
  describe('protocol id', () => {
    it('should not allow adding protocols without permission', async () => {
      await expect(factory.registerProtocolId(BEEFY_PROTOCOL_ID, 'Beefy')).to.be.revertedWith('BAL#401');
    });

    context('with no registered protocols', () => {
      it('should revert when asking for an unregistered protocol name', async () => {
        await expect(factory.getProtocolName(BEEFY_PROTOCOL_ID)).to.be.revertedWith('Protocol ID not registered');
      });
    });
