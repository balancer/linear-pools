...
describe('GearboxLinearPoolFactory', function () {
  ...
  const GEARBOX_PROTOCOL_NAME = 'GEARBOX';
  ...
  beforeEach('deploy factory & tokens', async () => {
    ...
    const gearboxVault = await deployPackageContract('MockGearboxVault', { args: [mainToken.address] });
    const wrappedTokenInstance = await deployPackageContract('MockGearboxDieselToken', {
      args: ['dDAI', 'dDAI', 18, gearboxVault.address],
    });
    ...
    factory = await deployPackageContract('GearboxLinearPoolFactory', {
      args: [
        vault.address,
        ZERO_ADDRESS,
        queries.address,
  ...
  async function createPool(): Promise<Contract> {
    ...
    const dDAI = await tokens.getTokenBySymbol('dDAI');
    ...
    expectEvent.inReceipt(receipt, 'GearboxLinearPoolCreated', {
      pool: event.args.pool,
      protocolId: GEARBOX_PROTOCOL_ID,
    });

    return getPackageContractDeployedAt('GearboxLinearPool', event.args.pool);
  }
  ...
  describe('constructor arguments', () => {
    ...
    it('registers tokens in the vault', async () => {
      ...
      expect(poolTokens.tokens).to.include(dDAI.address);
    ...
    it('sets a rebalancer as the asset manager', async () => {
      ...
      const rebalancer = await getPackageContractDeployedAt('GearboxLinearPoolRebalancer', assetManager);
    ...
    it('sets wrapped token', async () => {
      const dDAI = await tokens.getTokenBySymbol('dDAI');
      expect(await pool.getWrappedToken()).to.equal(dDAI.address);
    });
  ...
  describe('protocol id', () => {
    it('should not allow adding protocols without permission', async () => {
      await expect(factory.registerProtocolId(GEARBOX_PROTOCOL_ID, 'GEARBOX')).to.be.revertedWith('BAL#401');
    });

    context('with no registered protocols', () => {
      it('should revert when asking for an unregistered protocol name', async () => {
        await expect(factory.getProtocolName(GEARBOX_PROTOCOL_ID)).to.be.revertedWith('Protocol ID not registered');
      });
    });

    // TODO These tests are important only for Gearbox Linear Pool, but should be tested properly.
    ...
    //     await factory.connect(admin).registerProtocolId(GEARBOX_PROTOCOL_ID, GEARBOX_PROTOCOL_NAME);
    ...
    //     expectEvent.inReceipt(await tx.wait(), 'GearboxLinearPoolProtocolIdRegistered', {
    ...
    //     expect(await factory.getProtocolName(GEARBOX_PROTOCOL_ID)).to.equal(GEARBOX_PROTOCOL_NAME);
