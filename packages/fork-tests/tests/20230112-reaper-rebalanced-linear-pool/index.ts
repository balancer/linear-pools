...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const factory = await task.deployAndVerify('ReaperLinearPoolFactory', args, from, force);
  ...
  if (task.mode === TaskMode.LIVE) {
    ...
    // ReaperLinearPools require an rfToken
    const mockReaperVaultArgs = ['DO NOT USE - Mock rfToken', 'TEST', 18, input.WETH];
    const mockReaperVault = await task.deployAndVerify('mockReaperVault', mockReaperVaultArgs, from, force);
    ...
    const mockPoolArgs = {
      ...
      wrappedToken: mockReaperVault.address,
    ...
    if (force || !task.output({ ensure: false })['MockReaperLinearPool']) {
      const PROTOCOL_ID = 0;
      ...
      await task.save({ MockReaperLinearPool: mockPoolAddress });
    }

    const mockPool = await task.instanceAt('ReaperLinearPool', task.output()['MockReaperLinearPool']);
    ...
    await task.verify('ReaperLinearPool', mockPool.address, [mockPoolArgs]);

    // We can also verify the Asset Manager
    await task.verify('ReaperLinearPoolRebalancer', assetManagerAddress, [input.Vault, input.BalancerQueries]);
  }
};
