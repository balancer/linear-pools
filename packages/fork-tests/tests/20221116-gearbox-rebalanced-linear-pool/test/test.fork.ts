...
describeForkTest('GearboxLinearPoolFactory', 'mainnet', 15989794, function () {
  ...
  before('run task', async () => {
    task = new Task('20221116-gearbox-rebalanced-linear-pool', TaskMode.TEST, getForkedNetwork(hre));
  ...
  describe('create, join, and rebalance', () => {
    it('deploy a linear pool', async () => {
      const tx = await factory.create('', '', USDC, dUSDC, INITIAL_UPPER_TARGET, SWAP_FEE_PERCENTAGE, owner.address);
