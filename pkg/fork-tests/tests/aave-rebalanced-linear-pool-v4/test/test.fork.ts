...
describeForkTest('AaveLinearPoolFactory', 'mainnet', 15225000, function () {
  ...
  before('run task', async () => {
    task = new Task('aave-rebalanced-linear-pool-v4', TaskMode.TEST, getForkedNetwork(hre));
  ...
  describe('create and check getters', () => {
    ...
    it('check factory version', async () => {
      const expectedFactoryVersion = {
        ...
        version: 4,
        deployment: 'aave-rebalanced-linear-pool-v4',
      };
  ...
  describe('rebalancer query protection', async () => {
    it('reverts with a malicious lending pool', async () => {
      ...
      await setCode(USDT_LENDING_POOL, getExternalPackageArtifact('linear-pools/MockAaveLendingPool').deployedBytecode);
      const mockLendingPool = await getExternalPackageDeployedAt('linear-pools/MockAaveLendingPool', USDT_LENDING_POOL);
