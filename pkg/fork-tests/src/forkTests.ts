...
export function describeForkTest(name: string, forkNetwork: string, blockNumber: number, callback: () => void): void {
  describe(name, () => {
    _describeBody(forkNetwork, blockNumber, callback);
  });
}

describeForkTest.only = function (name: string, forkNetwork: string, blockNumber: number, callback: () => void): void {
...
describeForkTest.skip = function (name: string, forkNetwork: string, blockNumber: number, callback: () => void): void {
  describe.skip(name, () => {
    _describeBody(forkNetwork, blockNumber, callback);
  });
};

function _describeBody(forkNetwork: string, blockNumber: number, callback: () => void) {
