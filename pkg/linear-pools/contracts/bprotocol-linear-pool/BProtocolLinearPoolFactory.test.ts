...
describe('BProtocolLinearPoolFactory', function () {
  ...
  beforeEach('deploy factory & tokens', async () => {
    ...
    factoryVersion = JSON.stringify({
      name: 'BProtocolLinearPoolFactory',
      version: '1',
    ...
    factory = await deployPackageContract('BProtocolLinearPoolFactory', {
      args: [
        vault.address,
        ZERO_ADDRESS,
        queries.address,
  ...
  async function createPool(): Promise<Contract> {
    const LUSD = await tokens.getTokenBySymbol('LUSD');
    const wbamm = await tokens.getTokenBySymbol('WBAMM');
    ...
    expectEvent.inReceipt(receipt, 'BProtocolLinearPoolCreated', {
      pool: event.args.pool,
      protocolId: BPROTOCOL_PROTOCOL_ID,
    });

    return getPackageContractDeployedAt('BProtocolLinearPool', event.args.pool);
  }
  ...
  describe('constructor arguments', () => {
    ...
    it('registers tokens in the vault', async () => {
      ...
      expect(poolTokens.tokens).to.include(LUSD.address);
      expect(poolTokens.tokens).to.include(wbamm.address);
    ...
    it('sets main token', async () => {
      const LUSD = await tokens.getTokenBySymbol('LUSD');
      expect(await pool.getMainToken()).to.equal(LUSD.address);
    });

    it('sets wrapped token', async () => {
      const wbamm = await tokens.getTokenBySymbol('WBAMM');
      expect(await pool.getWrappedToken()).to.equal(wbamm.address);
    });
